...
class SrVector2
	...
	*/
	SR_INLINE explicit SrVector2(float a);
	...
	*/
	SR_INLINE SrVector2(float nx, float ny);
	...
	*/
	SR_INLINE SrVector2(const float v[]);
...
//legacy methods:
	...
	*/
	...
	\brief reads 2 consecutive values from the ptr passed
	*/
	SR_INLINE void  set(float, float);
	...
	*/
	...
	\brief this = s * a;
	*/
	SR_INLINE void  multiply(float s,  const SrVector2 & a);
	...
	*/
	...
	\brief this = s * a + b;
	*/
	SR_INLINE void  multiplyAdd(float s, const SrVector2 & a, const SrVector2 & b);
	...
	*/
	SR_INLINE float dot(const SrVector2 &other) const;
	/**
	\brief returns the scalar cross product of this and other.
	*/
	SR_INLINE float cross(const SrVector2 &other) const;
	...
	*/
	SR_INLINE float distance(const SrVector2 &) const;

	/**
	\brief returns (this - other).magnitudeSquared();
	*/
	SR_INLINE float distanceSquared(const SrVector2 &v) const;
	...
	*/
	SR_INLINE bool equals(const SrVector2 &, float epsilon) const;
	...
	*/
	SR_INLINE SrVector2 operator *(float f) const;
	/**
	...
	*/
	SR_INLINE SrVector2&operator *=(float f);
	/**
	\brief scalar division
	*/
	SR_INLINE SrVector2&operator /=(float f);
	...
	\brief dot product
	*/
	SR_INLINE float      operator|(const SrVector2& v) const;
...
//implementations:
SrVector2::SrVector2(float v) : x(v), y(v)
	{
	}

SR_INLINE SrVector2::SrVector2(float _x, float _y) : x(_x), y(_y)
	{
	}


SR_INLINE SrVector2::SrVector2(const float v[]) : x(v[0]), y(v[1])
	{
	}
...
SR_INLINE const float* SrVector2::get() const
	{
	return &x;
	}


SR_INLINE float* SrVector2::get()
	{
	return &x;
	}
...
SR_INLINE float& SrVector2::operator[](int index)
	{
...
SR_INLINE float  SrVector2::operator[](int index) const
	{
	SR_ASSERT(index>=0 && index<=1);
	return (&x)[index];
	}
...
SR_INLINE void SrVector2::setx(const float & d) 
	{ 
	x = d; 
	}

 
SR_INLINE void SrVector2::sety(const float & d) 
	{ 
	y = d; 
	}
...
SR_INLINE void  SrVector2::set(float _x, float _y)
	{
	this->x = _x;
	this->y = _y;
	}
...
SR_INLINE void SrVector2::set(float v)
	{
	x = v;
	y = v;
	}
...
SR_INLINE void  SrVector2::multiply(float s,  const SrVector2 & a)
	{
	x = a.x * s;
	y = a.y * s;
	}
...
SR_INLINE void  SrVector2::multiplyAdd(float s, const SrVector2 & a, const SrVector2 & b)
	{
	x = s * a.x + b.x;
	y = s * a.y + b.y;
	}
...
SR_INLINE float SrVector2::normalize()
	{
	float m = magnitude();
	if (m)
		{
...
SR_INLINE void SrVector2::setMagnitude(float length)
	{
	float m = magnitude();
	if(m)
		{
...
SR_INLINE float SrVector2::dot(const SrVector2 &v) const
	{
		return x * v.x + y * v.y;
	}
SR_INLINE float SrVector2::cross(const SrVector2 &other) const
	{
...
SR_INLINE float SrVector2::magnitude() const
	{
	return SrMath::sqrt(x * x + y * y);
	}

 
SR_INLINE float SrVector2::magnitudeSquared() const
	{
	return x * x + y * y;
	}

 
SR_INLINE float SrVector2::distance(const SrVector2 & v) const
	{
	float dx = x - v.x;
	float dy = y - v.y;
...
SR_INLINE float SrVector2::distanceSquared(const SrVector2 &v) const
	{
	float dx = x - v.x;
	float dy = y - v.y;
	return dx * dx + dy * dy;
...
SR_INLINE bool SrVector2::equals(const SrVector2 & v, float epsilon) const
	{
	return 
...
SR_INLINE SrVector2 SrVector2::operator *(float f) const
	{
...
SR_INLINE SrVector2 SrVector2::operator /(float f) const
	{
		f = float(1.0) / f; return SrVector2(x * f, y * f);
	}
...
SR_INLINE SrVector2& SrVector2::operator *=(float f)
	{
	x *= f;
	y *= f;
...
SR_INLINE float SrVector2::operator|(const SrVector2& v) const
	{
	return x * v.x + y * v.y;
	}
...
SR_INLINE SrVector2 operator *(float f, const SrVector2& v)
	{
